...
const EMPTY: Omit<Project, 'id' | 'position'> = {
  ...
  description: '', tags: [], github: '', url: '', video: '', cover: '', gallery: [],
};
...
export default function AdminPage() {
  ...
  const saveEditing = async () => {
    ...
    const payload = { ...editing, gallery };

    const r = await fetch('/api/projects', {
      method: 'POST', headers, body: JSON.stringify(payload),
    });
    if (r.ok) {
      showToast(isNew ? 'Project added ✓' : 'Saved ✓');
      await fetchProjects();
      setEditing(null);
    } else {
  ...
  const deleteProject = async (id: number) => {
    ...
    const r = await fetch(`/api/projects?id=${id}`, { method: 'DELETE', headers });
  ...
  const moveUp = async (i: number) => {
    ...
    await fetch('/api/projects', {
      ...
      body: JSON.stringify(updated.map((p, idx) => ({ id: p.id, position: idx }))),
    });
  };
  ...
  if (!authed) return (
    <div style={S.loginWrap}>
      <div style={S.loginBox}>
        ...
        <div style={S.loginSub}>Projects CMS</div>
        <input
          style={{ ...S.inp, marginBottom: 8 }}
          type="password" placeholder="Password"
          value={passInput} onChange={e => setPass(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && login()} autoFocus
        />
        {passErr && <div style={S.err}>Wrong password</div>}
        <button style={{ ...S.btnPrimary, width: '100%', marginTop: 12 }} onClick={login}>Log in →</button>
      </div>
    </div>
  );
  ...
  if (editing) return (
    <div style={S.page}>
      <div style={S.topbar}>
        ...
        <button style={S.btnPrimary} onClick={saveEditing} disabled={saving}>
      ...
      <div style={S.formWrap}>
        <div style={S.formGrid}>
          <div style={S.formCol}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <Field label="Size">
                <select style={S.inp} value={editing.size || 'small'} onChange={e => setEditing({ ...editing, size: e.target.value as 'large' | 'small' })}>
            ...
            <Field label="Video URL (YouTube / Vimeo)">
              <input style={S.inp} value={editing.video || ''} onChange={e => setEditing({ ...editing, video: e.target.value })} placeholder="https://youtube.com/watch?v=..." />
            </Field>
          </div>
          ...
          <div style={S.formCol}>
            ...
            {editing.cover && (
              <img src={editing.cover} alt="cover"
            ...
            <Field label="Gallery images — one URL per line">
              <textarea
                style={{ ...S.inp, minHeight: 120, resize: 'vertical', fontFamily: 'IBM Plex Mono, monospace', fontSize: 12 }}
                value={galInput} onChange={e => setGalInput(e.target.value)}
                placeholder={'https://img1.com/photo.jpg\nhttps://img2.com/photo.jpg\nhttps://img3.com/photo.jpg'}
              />
            </Field>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 8 }}>
          ...
          <button style={S.btnPrimary} onClick={saveEditing} disabled={saving}>{saving ? 'Saving...' : 'Save project'}</button>
        </div>
      </div>
  ...
  return (
    <div style={S.page}>
      <div style={S.topbar}>
        <span style={S.topTitle}>Projects CMS <span style={{ fontSize: 13, color: '#5a5a7a', fontWeight: 400 }}>({projects.length})</span></span>
      ...
      <div style={S.listWrap}>
        {loading && <div style={S.muted}>Loading from Supabase...</div>}
        {!loading && projects.length === 0 && (
          <div style={{ ...S.muted, textAlign: 'center', padding: '4rem' }}>
        ...
        {projects.map((p, i) => (
          <div key={p.id} style={S.projRow}>
            {p.cover
              ...
              : <div style={{ ...S.rowThumb, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, color: '#5a5a7a' }}>◈</div>
            }
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>
                ...
                {p.url && <span style={S.badge}>↗ live</span>}
              </div>
            </div>
            ...
            {p.gallery?.length > 0 && (
              <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                ...
                {p.gallery.length > 3 && (
                  <div style={{ width: 44, height: 44, borderRadius: 6, background: 'rgba(255,255,255,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#5a5a7a' }}>
                    +{p.gallery.length - 3}
                  </div>
                )}
              </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 16 }}>
      <label style={{ fontFamily: 'IBM Plex Mono,monospace', fontSize: 11, color: '#5a5a7a', letterSpacing: '2px', textTransform: 'uppercase' as const }}>{label}</label>
      {children}
    </div>
  );
}
...
const S: Record<string, React.CSSProperties> = {
  ...
  listWrap:   { maxWidth: 920, margin: '0 auto', padding: '32px 24px', display: 'flex', flexDirection: 'column', gap: 12 },
  ...
  muted:      { color: '#5a5a7a', fontSize: 14 },
  ...
  loginTitle: { fontSize: 18, fontWeight: 800, marginBottom: 4, color: '#d4d4e8' },
